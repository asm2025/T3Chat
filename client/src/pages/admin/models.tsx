...
export function AdminModels() {
    ...
    const fetchModels = useCallback(async () => {
        try {
            ...
            const data = await api.get<ModelsResponse>("/admin/models", params);
            setModels(data.data);
        } catch (error) {
    ...
    const handleEnable = async (modelId: string) => {
        try {
            await api.post(`/admin/models/${modelId}/enable`);
    ...
    const handleDisable = async (modelId: string) => {
        try {
            await api.post(`/admin/models/${modelId}/disable`);
    ...
    const handleDeprecate = async (modelId: string) => {
        ...
        try {
            await api.post(`/admin/models/${modelId}/deprecate`);
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-2">Manage AI model configurations</p>
                    </div>
                    <div className="flex gap-2">
                        <Button variant="outline">Scan Providers</Button>
                        <Button>
                    ...
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            ...
                            <div className="relative">
                                ...
                                <Input placeholder="Search models..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-8 w-64" />
                            </div>
                        </div>
                    </CardHeader>
                    <CardContent>
                                    ...
                                    <div key={model.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {model.disabled && <Badge variant="destructive">Disabled</Badge>}
                                        ...
                                        <div className="flex items-center gap-2">
                                            {model.disabled ? (
                                                <Button variant="outline" size="sm" onClick={() => handleEnable(model.id)}>
                                                    <Power className="h-4 w-4" />
                                                </Button>
                                            ) : (
                                                <Button variant="outline" size="sm" onClick={() => handleDisable(model.id)}>
                                                    <PowerOff className="h-4 w-4" />
                                                </Button>
                                            )}
                                            {!model.deprecatedAt && (
                                                <Button variant="outline" size="sm" onClick={() => handleDeprecate(model.id)}>
