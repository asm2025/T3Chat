...
import type { ConversationRequest } from "@/types/api";

export function useStreamingChat() {
    const [streaming, setStreaming] = useState(false);
    const [error, setError] = useState<Error | null>(null);

    const sendMessage = useCallback(async (request: ConversationRequest, onChunk: (chunk: string) => void, onComplete: () => void) => {
        try {
            setStreaming(true);
            setError(null);
