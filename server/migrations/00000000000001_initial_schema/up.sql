...
CREATE INDEX idx_agents_ai_model_id ON agents(ai_model_id);
...
CREATE TABLE conversations (
    ...
    current_ai_model_id UUID NOT NULL REFERENCES ai_models(id) ON DELETE RESTRICT,
...
CREATE INDEX idx_conversations_current_model ON conversations(current_ai_model_id);
...
CREATE TABLE messages (
    ...
    -- User can switch models mid-conversation, so we track which model generated THIS message
    ai_model_id UUID REFERENCES ai_models(id) ON DELETE SET NULL,  -- NULL for user messages
...
CREATE INDEX idx_messages_ai_model_id ON messages(ai_model_id) WHERE ai_model_id IS NOT NULL;
...
CREATE INDEX idx_presets_ai_model_id ON presets(ai_model_id);
...
CREATE INDEX idx_agent_tools_order ON agent_tools(agent_id, order_index);
...
CREATE INDEX idx_assistant_tools_order ON assistant_tools(assistant_id, order_index);
...
CREATE TABLE tool_calls (
    ...
    -- Execution
    arguments TEXT,  -- serialized function arguments (JSON string)
    result TEXT,  -- serialized function result (JSON string)
...
CREATE TABLE transactions (
    ...
    -- Model info (snapshot for historical accuracy - model pricing may change)
    ai_model_id UUID NOT NULL REFERENCES ai_models(id) ON DELETE RESTRICT,
...
CREATE INDEX idx_transactions_ai_model_id ON transactions(ai_model_id, created_at DESC);
