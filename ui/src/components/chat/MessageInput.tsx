...
export const MessageInput = forwardRef<MessageInputRef, MessageInputProps>(
    ...
    useLayoutEffect(() => {
        ...
        const container = document.getElementById("chat-footer-portal");
    ...
    useEffect(() => {
        ...
        try {
            const storedValue = localStorage.getItem("t3chat:termsAccepted");
            setHasAcceptedTerms(storedValue === "true");
        } catch (err) {
            console.error("Failed to read terms acceptance state:", err);
        }
    }, []);
    ...
    const handleAcceptTerms = () => {
        ...
        if (typeof window === "undefined") return;
        try {
            localStorage.setItem("t3chat:termsAccepted", "true");
        } catch (err) {
            console.error("Failed to store terms acceptance state:", err);
        }
    };
            ...
            {!hasAcceptedTerms && (
                <div className="pointer-events-auto mx-auto w-fit">
                    <div className="m-3 -mb-px flex justify-center">
                        <div className="prose border-secondary/40 bg-chat-background/50 text-secondary-foreground/80 blur-fallback:bg-chat-background flex flex-col gap-3 rounded-t-md border p-4 text-sm backdrop-blur-md sm:flex-row sm:items-center sm:justify-center">
                            <span>
                                Make sure you agree to our{" "}
                                <Link className="text-foreground hover:text-primary dark:hover:text-muted-foreground" to="/terms-of-service">
                                    Terms
                                </Link>{" "}
                                and our{" "}
                                <Link className="text-foreground hover:text-primary dark:hover:text-muted-foreground" to="/privacy-policy">
                                    Privacy Policy
                                </Link>
                            </span>
                            <Button
                                ...
                                className="focus-visible:ring-ring inline-flex h-8 items-center justify-center gap-2 rounded-md bg-secondary/50 px-3 text-xs font-medium text-secondary-foreground transition-colors hover:bg-secondary focus-visible:ring-1 focus-visible:outline-hidden sm:ml-4">
                                I agree
                            </Button>
                        </div>
                    </div>
    ...
    if (portalNode) {
        return createPortal(composer, portalNode);
    }
