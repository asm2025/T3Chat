...
const createChatsSlice = (set: any, get: any): ChatsSlice => ({
  ...
  updateChat: (id, updates) => {
    const state = get();
    state.setChats(
      state.chats.map((chat) => (chat.id === id ? { ...chat, ...updates } : chat))
    );
  },
  
  removeChat: (id) => {
    const state = get();
    state.setChats(state.chats.filter((chat) => chat.id !== id));
  },
});
...
const createChatSlice = (set: any, get: any): ChatSlice => ({
  ...
  addMessage: (message) => set((state: ChatSlice) => ({
    messages: [...state.messages, message],
  })),
  updateMessage: (messageId, updates) => set((state: ChatSlice) => ({
    messages: state.messages.map((msg) =>
      msg.id === messageId ? { ...msg, ...updates } : msg
    ),
  })),
  removeMessage: (messageId) => set((state: ChatSlice) => ({
...
const createFeaturesSlice = (set: any, get: any): FeaturesSlice => ({
  ...
  setFeature: (feature, enabled) => set((state: FeaturesSlice) => ({
...
const createUserApiKeysSlice = (set: any, get: any): UserApiKeysSlice => ({
  ...
  deleteApiKey: async (id) => {
    ...
    state.setApiKeys(state.apiKeys.filter((k) => k.id !== id));
  },
});
...
export const useModels = () => useAppStore(
  useShallow((state) => ({
    models: state.models,
...
export const useChats = () => useAppStore(
  useShallow((state) => ({
    chats: state.chats,
...
export const useChat = () => useAppStore(
  useShallow((state) => ({
...
export const useUserApiKeys = () => useAppStore(
  useShallow((state) => ({
    keys: state.apiKeys,
