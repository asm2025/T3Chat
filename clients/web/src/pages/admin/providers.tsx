...
export function AdminProviders() {
    ...
    const fetchProviders = useCallback(async () => {
        try {
            ...
            const data = await api.get<ProvidersResponse>("/admin/providers", params);
    ...
    const handleEnable = async (providerId: string) => {
        try {
            await api.post(`/admin/providers/${providerId}/enable`);
    ...
    const handleDisable = async (providerId: string) => {
        try {
            await api.post(`/admin/providers/${providerId}/disable`);
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-2">Manage AI provider configurations</p>
                    </div>
                    <Button>
                    ...
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription>{providers.length} providers</CardDescription>
                            </div>
                            <div className="relative">
                                <Search className="absolute left-2 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                                <Input placeholder="Search providers..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-8 w-64" />
                            </div>
                        </div>
                    </CardHeader>
                    <CardContent>
                                    ...
                                    <div key={provider.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {provider.disabled && <Badge variant="destructive">Disabled</Badge>}
                                        ...
                                        <div className="flex items-center gap-2">
                                            {provider.disabled ? (
                                                <Button variant="outline" size="sm" onClick={() => handleEnable(provider.id)}>
                                                    <Power className="h-4 w-4" />
                                                </Button>
                                            ) : (
                                                <Button variant="outline" size="sm" onClick={() => handleDisable(provider.id)}>
