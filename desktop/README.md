...
The desktop client uses Flutter for the UI and a Rust core for API communication via `flutter_rust_bridge`.
...
If dependency resolution fails, reset generated folders and try again:
...
```bash
cd desktop/native/t3chat_core
flutter_rust_bridge_codegen generate
```

Or from the desktop root directory:

```bash
cd desktop
flutter_rust_bridge_codegen generate --config-file native\\t3chat_core\\flutter_rust_bridge.yaml
```
...
Run the code generation command (see Build Steps section above):
