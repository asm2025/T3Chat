...
T3 Chat is an AI chat application that enables users to select from multiple AI language models and providers, engaging in ChatGPT-like chats. The application abstracts the complexity of different AI provider APIs behind a unified interface, allowing users to seamlessly switch between models and maintain chat history.
...
-   `GET /api/models/:id` - Get model details (public)
-   `POST /api/chat` - Send chat message (non-streaming, authenticated)
-   `POST /api/chat/stream` - Send chat message (streaming, authenticated)
-   `GET /api/keys` - Get user's API keys (authenticated)
-   `POST /api/keys` - Add/update API key (authenticated)
-   `DELETE /api/keys/:id` - Delete API key (authenticated)
...
-   `ChatBranchRepository` - Manage chat branches

#### 2.3 API Endpoints

-   `GET /api/chats` - List user's chats (paginated, authenticated)
-   `POST /api/chats` - Create new chat (authenticated)
-   `GET /api/chats/:id` - Get chat with messages (authenticated)
-   `PUT /api/chats/:id` - Update chat (title, etc., authenticated)
-   `DELETE /api/chats/:id` - Delete chat (soft delete, authenticated)
-   `GET /api/chats/:id/messages` - Get messages for a chat (authenticated)
-   `POST /api/chats/:id/messages` - Add message to chat (authenticated)
-   `GET /api/chats/:id/branches` - Get chat branches (authenticated)
...
-   Quick start guide
-   Sample chats to demonstrate capabilities
...
**Objective**: Allow users to upload files (images and PDFs) to enhance chats.
...
-   `GET /api/attachments/:id` - Download file (authenticated)
-   `GET /api/attachments/:id/thumbnail` - Get thumbnail (authenticated)
...
-   `POST /api/images/generate` - Generate image (authenticated)
-   `GET /api/images/:id` - Get generated image (authenticated)
...
-   `POST /api/chat/stream/resume` - Resume interrupted stream (authenticated)
-   `GET /api/chat/stream/:id/status` - Get stream status (authenticated)
...
**Objective**: Create alternative chat paths.
...
-   `BranchTree` - Tree view of chat branches
-   `CreateBranch` - Button to create new branch from message

#### 5.3 API Endpoints

-   `POST /api/chats/:id/branches` - Create branch (authenticated)
-   `GET /api/chats/:id/branches` - List branches (authenticated)
-   `GET /api/chats/:id/branches/:branch_id` - Get branch messages (authenticated)
-   `DELETE /api/chats/:id/branches/:branch_id` - Delete branch (authenticated)
...
**Objective**: Share chats with others.
...
-   `POST /api/chats/:id/share` - Create share link (authenticated)
-   `GET /api/shared/:token` - Get shared chat (public)
-   `DELETE /api/chats/:id/share` - Revoke share (authenticated)
-   `GET /api/chats/:id/share/stats` - Get share statistics (authenticated)
...
-   `POST /api/search` - Perform web search (authenticated)
...
-   `POST /api/keys` - Add API key (authenticated)
-   `PUT /api/keys/:id` - Update API key (authenticated)
-   `DELETE /api/keys/:id` - Delete API key (authenticated)
-   `POST /api/keys/:id/validate` - Validate API key (authenticated)
...
-   Public endpoints (e.g., `/api/models`, `/api/shared/:token`) without authentication
