...
- **LibreChat behavior** (observed in `packages/api/src/utils/env.ts`):
  - Resolves `${ENV_VAR}` *when using the values*, not by rewriting the whole YAML at load-time.
...
- **Work**
  - Implement `LibreChatConfig` with `serde`:
...
- **Goal**: “Load or ignore, never crash” unless *server cannot run for other reasons* (DB, etc.)
...
- Current T3Chat endpoint: `GET /api/v1/config/startup` returns a small object.
- Add/adjust to mirror LibreChat’s “startup config contract”:
  - New endpoint recommended: `GET /api/v1/config` (or keep `/startup` but align fields).
...
- T3Chat should:
  ...
  - Optionally add an admin-only endpoint `POST /api/v1/admin/config/reload` to re-read YAML and rebuild catalogs.
...
flowchart TD
  ...
  buildAppConfig --> serveStartup[ServeGET_/api/v1/config]
  buildAppConfig --> buildCatalog[BuildModelCatalog]
  buildCatalog --> serveModels[ServeGET_/api/v1/models]
