...
-   Add first‑class support for **OpenRouter** and **ChatLLM** as configurable providers.
...
-   **Config‑driven model system**
    ...
    -   Defines:
        ...
        -   Provider/endpoint definitions (OpenAI, Anthropic, Google, Bedrock, Azure, OpenRouter, ChatLLM, custom HTTP providers).
...
-   **Backend API surface**
    ...
    -   `GET /api/config/startup` (or similar) returns:
        -   Interface flags.
        -   Model specs.
...
-   **Task B1: Define Rust config structs & schema**
    ...
    -   Design a `Config` Rust type that mirrors relevante LibreChat concepts but is idiomatic for T3Chat:
        ...
        -   `endpoints` or `providers` section, with entries like:
            -   `openai`, `anthropic`, `google`, `bedrock`, `azure`, `openrouter`, `chatllm`, and an optional `custom` list for generic HTTP providers.
        -   Each provider config should support:
            -   `api_key` reference (string that may include `${ENV_VAR}` syntax).
            -   `base_url` (optional for built‑ins, required for custom, OpenRouter, ChatLLM).
            -   `models` sub‑object:
                -   `default: [model_id, ...]`
...
-   **Task B3: Example configuration file**
    -   Create `t3chat.example.yaml` containing:
        -   Documented sections for:
            ...
            -   OpenRouter example.
            -   ChatLLM example (see section 2.4).
...
### 2.4 New Providers: OpenRouter & ChatLLM
...
-   **Task B11: ChatLLM support**
    -   Research phase (no code yet):
        -   Confirm current ChatLLM provider capabilities and finalize its config and API shape.
        ...
        -   Decide whether we treat it as:
            -   A **single logical provider** (e.g. `chatllm` with one backend-managed API key), or
            -   A simple “meta‑endpoint” over one of the existing providers.
    -   Design phase:
        -   Specify config shape in `t3chat.yaml`, e.g.:
            -   `chatllm: { api_key, base_url, models: { default: [...], fetch: bool } }`.
...
-   **Task B12: Startup config endpoint**
    ...
    -   Implement `GET /api/config/startup` that returns:
...
-   **Task B13: Models listing endpoint**
    -   Implement `GET /api/models` requiring authenticated user:
...
-   **Task F3: Local state integration**
    -   Decide where to store:
        -   Selected `{provider, model}` for current chat.
...
-   **Task F4: High‑level UX design**
    ...
    -   Design how users pick models:
        ...
        -   Per‑chat selector on the **Chat** page header or input area.
    -   Handle two flows:
...
-   **Task F5: Model selector component**
    ...
    -   Implement a `ModelSelector` similar to LibreChat’s:
        ...
        -   Shows provider icons (OpenAI, Anthropic, Google, Bedrock, Azure, OpenRouter, ChatLLM, etc.).
    -   Integrate with:
        -   Chat creation and chat switching.
...
-   **Task F7: OpenRouter & ChatLLM UI**

    -   Add recognizable branding/icons for:
        -   OpenRouter.
        -   ChatLLM.
    -   If ChatLLM uses a “default” model id to indicate routing (omit model field):
...
-   **Task F8: Error & status feedback**
    -   Surface when:
        ...
        -   A requested model is no longer available (config changed since chat created).
...
-   **Backend Track B (Providers & Model Catalog)**
    ...
    -   Delivers provider trait, provider registry, dynamic discovery, and OpenRouter/ChatLLM integrations.
...
-   Confirm ChatLLM capabilities and finalize its config and API shape.
