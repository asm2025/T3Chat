...
-   **Backend developer**
    ...
    -   Delivers YAML config loading, provider registry, model catalog, DB alignment, and `/config/startup` + `/models` API surfaces (plus OpenRouter/ChatLLM backends).
...
-   **Week 3–4**: Integration, end‑to‑end testing, polish, and OpenRouter/ChatLLM wiring.
...
-   **Implementation steps**
    ...
    1. **Define Rust config structs (Task B1)**
        ...
        - In `server/src/config/t3chat.rs`, define:
            ...
            - `providers: HashMap<String, ProviderConfig>` or explicit struct with fields `openai`, `anthropic`, `google`, `bedrock`, `azure`, `openrouter`, `chatllm`, `custom: Vec<CustomProviderConfig>`.
    ...
    4. **Create example configuration (Task B3)**
        ...
        - Add `t3chat.example.yaml` at repo root with:
            ...
            - Sections:
                ...
                - `chatllm` example as an OpenAI-compatible custom provider.
...
### 2. Provider Registry & Model Catalog – Tasks B4–B6

-   **Goal**: Provide a normalized provider abstraction and a cached model catalog that merges config defaults with any provider‑discovered models.
...
-   **Files to add/update**
    ...
    -   **Provider registry**
        ...
        -   `server/src/ai/providers/chatllm.rs` (OpenAI-compatible provider).
    -   **Model catalog**
...
-   **Implementation steps**
    ...
    5. **ChatLLM design (Task B11)**
        - Document the config shape in `t3chat.example.yaml` for an OpenAI-compatible ChatLLM endpoint:
            - `apiKey`, `baseURL`, and `models` (with `default: ["default"]` to indicate provider-side routing when the model field is omitted).
        - Ensure the ChatLLM client is wired end-to-end through the provider manager.
...
-   **Files to update**
    ...
    -   `server/src/api/chat/*.rs`, `server/src/api/chats/*.rs`, `server/src/api/admin/*.rs` (wherever `{provider, model}` are accepted).
...
-   **Implementation steps**
    ...
    3. **Enforce config in chat flows (Task B9)**
        - In chat creation/update and message/agent APIs (e.g. `server/src/api/chat`, `server/src/api/chats`, `server/src/api/admin`):
...
-   **Files to add/update**
    ...
    -   `server/src/api/config/mod.rs` (new).
    -   `server/src/api/config/startup.rs` (new).
    -   `server/src/api/models/mod.rs` (already exists; may need to extend or add a new route).
    -   `server/src/main.rs` routing table (to add `/api/config/startup` and any new models route).

-   **Implementation steps**

    1. **Startup config endpoint (Task B12)**

        - Add `server/src/api/config/startup.rs`:
            - Handler `GET /api/config/startup` that:
    ...
    2. **Models listing endpoint (Task B13)**
        ...
        - There is already `/api/models` (see `server/src/api/models/mod.rs`) that returns DB `AiModel`s.
        - Decide on approach:
            - **Option A**: Extend existing `/api/models` to return a **config‑driven view** (e.g. `{ provider_key: ModelInfo[] }`) and deprecate the old shape.
            - **Option B**: Introduce a new endpoint, e.g. `GET /api/config/models`, that wraps `ModelCatalog` while leaving existing endpoints intact.
    ...
    3. **Backend testing**
        - Add/extend tests (or at least manual test scripts) to verify:
            ...
            - `/api/config/startup` and models listing endpoints return the expected JSON.
...
-   **Implementation steps**
    ...
    2. **API client for config & models (Task F2)**
        ...
        - In `ui/src/lib/t3-chat-client.ts`, add:
            - `export async function getStartupConfig(): Promise<StartupConfig> { /* GET /api/config/startup */ }`
...
-   **Implementation steps**
    ...
    1. **High‑level UX design (Task F4)**
        ...
        - Sketch how the selector is used in two contexts:
            ...
            - **Per‑chat** selector in chat header/tool‑bar (e.g. next to chat title).
        - Define flows:
...
-   **Goal**: Provide clear cues for OpenRouter/ChatLLM and good status/error feedback to users.
...
-   **Implementation steps**
    ...
    2. **Error & status feedback (Task F8)**
        ...
        - In `ModelSelector` and Models page:
            ...
            - If user opens a chat referencing a now‑disabled model:
...
-   **Checks**
    -   With a sample `t3chat.yaml`:
        -   **Startup**:
            -   App boots and `/api/config/startup` returns expected JSON.
