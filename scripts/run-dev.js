...
function detectEnvironmentConfiguration() {
    ...
    try {
        ...
        console.warn("Could not detect environment configuration, defaulting to local mode");
        return {
            useLocalFirebase: true,
            useLocalDatabase: true,
            isProduction: false,
...
async function startServices() {
    ...
    try {
        ...
        const child = spawn("npx", ["concurrently", "-c", serviceColors.join(","), "-n", serviceNames.join(","), "--handle-input", "--success", "first", ...commands], {
